...
def feature_selection_MSE(selected_feature):
    ...
    while True:
        ...
        model = linear_regression_vanilla(X_train, Y_train, X_columns=selected_feature, is_normalize=True)
    ...
    plt.subplot(3,1,1)
    plt.plot(time_hist, mse_hist, "-o", label="MSE")
    plt.ylabel("MSE"); plt.xlabel("Date")
    plt.legend()
    plt.subplot(3,1,2)
    ...
    plt.subplot(3,1,3)
    for i in range(len(model_result_hist)):
        plt.plot(selected_feature, model_result_hist[i][2], label=datetime.datetime.strftime(time_hist[i], "%Y-%m-%d"))
    plt.ylabel("p-value"); plt.xlabel("Feature")
    plt.legend(ncol=2)
    plt.xticks(rotation=45)
    plt.tight_layout()
    return np.mean(mse_hist)

def linear_model_diagnosis(selected_feature, year=2012, colinearity=False, nonlinearity=False, outlier=False, homoscedasticity=False, 
                           residual_normality_independence=False, feature_selection=False, visualize=False):
    ...
    time_hist = []; mse_hist = []; model_result_hist = []
    t_idx = np.searchsorted(time_axis, datetime.datetime(year, 1, 1))
    t_train_start_idx = t_idx - 251
    t_train_end_idx = t_idx
    t_test_start_idx = t_train_end_idx + 1
    t_test_end_idx = min(t_test_start_idx + 124, T - 1)
    ...
    Y_test = forward_beta[:, t_test_start_idx:(t_test_end_idx + 1)]
    data_test = np.concatenate((X_test, Y_test[:, :, np.newaxis]), axis=2)
    data_test = data_test.swapaxes(0, 1).reshape((-1, len(selected_feature)+1)) # make T become the principal axis so that during train-valid data split, respect causality
    data_test = data_test[~np.isnan(data_test).any(axis=1), :]
    data_test = data_test[~np.isinf(data_test).any(axis=1), :]
    X_test = data_test[:, 0:len(selected_feature)]; Y_test = data_test[:, [len(selected_feature)]]
    ...
    if residual_normality_independence:
...
class linear_regression_vanilla:
    ...
    def fit(self, cov_type="nonrobust", is_output=True):
        ...
        if is_output:
            ...
            plt.subplot(2,1,2)
            plt.plot(self.X_columns, self.ols.pvalues[1:], "-o")
            plt.xticks(rotation=45)
            plt.ylabel("p-value")
            plt.xlabel("Feature")
            plt.tight_layout()
    ...
    def predict(self, X):
        ...
        Y_pred = self.ols.predict(sm.add_constant(X, prepend=True, has_constant="add")).reshape(-1, 1)
        if self.is_normalize:
    ...
    def visualize_data(self):
        ...
        if self.p > 2:
            ...
            print("Projecting data by t-SNE...")
    ...
    def feature_selection_least_angle_regression(self, beta_threshold = 1e-3):
        ...
        for i in range(len(alpha_path)):
            ...
            if len(np.where(np.abs(coef) > beta_threshold)[0]) > feature_num:
                prev_feature_num = feature_num # prev_feature_num takes care of the case when feature_num incrases by more than 1
                feature_num = len(np.where(np.abs(coef) > beta_threshold)[0])
                for j in range(prev_feature_num+1, feature_num+1):
                    self.feature_selection_least_angle_regression_summary[j] = [list(np.where(np.abs(coef) > beta_threshold)[0]), R2_oos[i]]
                    print("Least Angle Regression: feature number: %d, alpha: %.4f, selected feature index: %s, R^2 (out-of-sample): %.4f" % (j, alpha, self.feature_selection_least_angle_regression_summary[j][0], R2_oos[i]))
                critical_alpha.append(alpha)
...
class ridge_lasso_regression:
    def __init__(self, X, Y, X_test=None, Y_test=None, X_columns=None, is_normalize=True, test_size_ratio=0.2, regularization="ridge"):
    ...
    def optimal_alpha(self, is_plot=True):
        alpha_list = np.logspace(-6, 6, num=100, base=10)
...
class principal_component_regression:
    ...
    def fit(self, factor_num=None, is_plot=False):
        if not factor_num:
        ...
        self.optimal_factor_number = factor_num
...
class partial_least_square_regression:
    ...
    def fit(self, factor_num=None, is_plot=True):
        if not factor_num:
            factor_num = self.optimal_factor_number(criterion="R^2 (out-of-sample)", is_plot=False)
        self.pls = sklearn.cross_decomposition.PLSRegression(n_components=factor_num)
    ...
    def optimal_factor_number(self, criterion = "R^2 (out-of-sample)", is_plot=True):
        factor_num = np.arange(1, self.p+1, 1)
        R2_oos = []
        for i in factor_num:
            pls = sklearn.cross_decomposition.PLSRegression(n_components=i)
            pls.fit(self.X_train, self.Y_train)
            Y_pred = pls.predict(self.X_test).reshape(-1, 1)
            R2_oos.append(1 - np.sum((self.Y_test - Y_pred) ** 2) / np.sum((self.Y_test - np.mean(self.Y_test)) ** 2))
        
        optimal_factor_num = factor_num[np.argmax(R2_oos)]

        if is_plot:
            plt.figure(figsize=(8, 3))
            plt.plot(factor_num, R2_oos, "o-")
            plt.xlabel("number of factors"); plt.ylabel(r"$R^2$ (out-of-sample)")
        return optimal_factor_num
...
class least_angle_regression:
    ...
    def fit(self, factor_num=None):
        if not factor_num:
            factor_num = self.optimal_factor_number(criterion="R^2 (out-of-sample)", is_plot=False)
        self.lar = sklearn.linear_model.Lars(n_nonzero_coefs=factor_num)
        self.lar.fit(self.X, self.Y)
        self.optimal_factor_number = factor_num

    def predict(self, X):
        if self.is_normalize:
            X = (X - self.X_mean) / self.X_std
        Y_pred = self.lar.predict(X).reshape(-1, 1)
        if self.is_normalize:
            Y_pred = Y_pred * self.Y_std + self.Y_mean
        return Y_pred
    ...
    def optimal_factor_number(self, criterion = "R^2 (out-of-sample)", is_plot=True):
        ...
        R2_oos = []
        for i in factor_num:
            lar = sklearn.linear_model.Lars(n_nonzero_coefs=i)
            lar.fit(self.X_train, self.Y_train)
            Y_pred = lar.predict(self.X_test).reshape(-1, 1)
            R2_oos.append(1 - np.sum((self.Y_test - Y_pred) ** 2) / np.sum((self.Y_test - np.mean(self.Y_test)) ** 2))
...
class random_forest_regression:
    def __init__(self, X, Y, X_test=None, Y_test=None, X_columns=None, is_normalize=True, test_size_ratio=0.2, is_gradient_boost=False):
        self.X = X; self.Y = Y.reshape(-1, 1)
        self.X_columns = X_columns if X_columns is not None else [f"feature_{i}" for i in range(X.shape[1])]
        self.is_normalize = is_normalize
        self.test_size_ratio = test_size_ratio
    ...
    def fit(self, params_dict=None):
        if params_dict is None:
            params_dict = self.optimal_params(is_plot=False)
        if self.is_gradient_boost:
            self.rf = xgboost.XGBRegressor(n_estimators=params_dict["n_estimators"], learning_rate=params_dict["learning_rate"], max_depth=params_dict["max_depth"])
            self.rf.fit(self.X, self.Y.flatten())
        else:
            self.rf = sklearn.ensemble.RandomForestRegressor(n_estimators=params_dict["n_estimators"], max_depth=params_dict["max_depth"])
            self.rf.fit(self.X, self.Y.flatten())
    ...
    def predict(self, X):
        ...
        Y_pred = self.rf.predict(X).reshape(-1, 1)
        if self.is_normalize:
            Y_pred = Y_pred * self.Y_std + self.Y_mean
        return Y_pred

    def optimal_params(self, criterion = "R^2 (out-of-sample)", is_plot=True):
        if self.is_gradient_boost:
            n_estimators = [100, 200, 500, 1000]
            learning_rate = [0.01, 0.05, 0.1, 0.2]
            max_depth = [3, 5, 10]
            R2_oos = dict(); R2_oos_max = 0
            for i in range(len(n_estimators)):
                for j in range(len(learning_rate)):
                    for k in range(len(max_depth)):
                        rf = xgboost.XGBRegressor(n_estimators=n_estimators[i], learning_rate=learning_rate[j], max_depth=max_depth[k])
                        rf.fit(self.X_train, self.Y_train.flatten())
                        Y_pred = rf.predict(self.X_test).reshape(-1, 1)
                        R2_oos_temp = 1 - np.sum((self.Y_test - Y_pred) ** 2) / np.sum((self.Y_test - np.mean(self.Y_test)) ** 2)
                        if R2_oos_temp > R2_oos_max:
                            R2_oos[(i, j, k)] = R2_oos_temp
                            self.opt_params = {"n_estimators": n_estimators[i], "learning_rate": learning_rate[j], "max_depth": max_depth[k], "R^2 (out-of-sample)": R2_oos_temp}
                            R2_oos_max = R2_oos_temp

        else:
            n_estimators_list = np.array([10, 50, 100, 200, 500])
            max_depth_list = np.array([None, 5, 10, 20, 50])
            self.opt_params = dict()
            R2_oos = np.zeros((len(n_estimators_list), len(max_depth_list)))
            for i in range(len(n_estimators_list)):
                for j in range(len(max_depth_list)):
                    rf = sklearn.ensemble.RandomForestRegressor(n_estimators=n_estimators_list[i], max_depth=max_depth_list[j])
                    rf.fit(self.X_train, self.Y_train.flatten())
                    Y_pred = rf.predict(self.X_test).reshape(-1, 1)
                    R2_oos[i, j] = 1 - np.sum((self.Y_test - Y_pred) ** 2) / np.sum((self.Y_test - np.mean(self.Y_test)) ** 2)
                    if R2_oos[i, j] > np.max(R2_oos):
                        self.opt_params["n_estimators"] = n_estimators_list[i]
                        self.opt_params["max_depth"] = max_depth_list[j]
                        self.opt_params["R^2 (out-of-sample)"] = R2_oos[i, j]

            if is_plot:
                plt.figure(figsize=(6, 6))
                plt.imshow(R2_oos, cmap="Reds", interpolation="nearest")
                plt.colorbar()
                plt.xticks(np.arange(len(max_depth_list)), max_depth_list)
