...
feature_pool = ["beta", 
                  "volume_avg_5d", "volume_avg_20d", "volume_spike",
...
Conclusion:
(1) Feature to remove:
    - price_high, price_low (high colinearity)
    - volume spike (low t-statistic, low colinearity)

(2) Feature to add:
    - price_high_low_diff
    - volume_avg_5d * beta
    - price_high_low_diff * beta
    - beta**2
'''

feature_dict["price_high_low_diff"] = feature_dict["price_high"] - feature_dict["price_low"]
feature_dict["volume_avg_5d*beta"] = np.multiply(feature_dict["volume_avg_5d"], feature_dict["beta"])
feature_dict["price_high_low_diff*beta"] = np.multiply(feature_dict["price_high_low_diff"], feature_dict["beta"])
...
feature_column = ["beta", 
                  "volume_avg_5d", "volume_avg_20d", 
                  "price_high_low_diff",
                  "volume_avg_5d*beta", "price_high_low_diff*beta", "beta**2"
                 ]
...
feature_validation.append(model.feature_selection_best_subset_summary[7][0])
...
file_name = os.path.join(os.getcwd(), "feature_vote_volume_price.pkl")
with open(file_name, "wb") as f:
